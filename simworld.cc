...
bool karte_t::can_raise_to(sint16 x, sint16 y, bool keep_water, sint8 hsw, sint8 hse, sint8 hne, sint8 hnw, int &cost, uint8 ctest) const
{
	...
	if(ist_in_kartengrenzen(x,y)) {
		...
		if (ok && h0_sw < hsw) {
			cost += hsw-h0_sw;
			ok = can_raise_to(x-1,y+1, keep_water, hsw-1, hsw-1, hsw, hsw-1, cost, 11);
		}
		// s
		if (ok && (h0_se < hse || h0_sw < hsw) && ((ctest&3)==3)) {
			const sint8 hs = max(hse, hsw) -1;
			ok = can_raise_to(x,y+1, keep_water, hs, hs, hse, hsw, cost, 3);
		}
		// se
		if (ok && h0_se < hse) {
			cost += hse-h0_se;
			ok = can_raise_to(x+1,y+1, keep_water, hse-1, hse-1, hse-1, hse, cost, 7);
		}
		// e
		if (ok && (h0_se < hse || h0_ne < hne) && ((ctest&6)==6)) {
			const sint8 he = max(hse, hne) -1;
			ok = can_raise_to(x+1,y, keep_water, hse, he, he, hne, cost, 6);
		}
		// ne
		if (ok && h0_ne < hne) {
			cost += hne-h0_ne;
			ok = can_raise_to(x+1,y-1, keep_water, hne, hne-1, hne-1, hne-1, cost, 14);
		}
		// n
		if (ok && (h0_nw < hnw || h0_ne < hne) && ((ctest&12)==12)) {
			const sint8 hn = max(hnw, hne) -1;
			ok = can_raise_to(x,y-1, keep_water, hnw, hne, hn, hn, cost, 12);
		}
		// nw
		if (ok && h0_nw < hnw) {
			cost += hnw-h0_nw;
			ok = can_raise_to(x-1,y-1, keep_water, hnw-1, hnw, hnw-1, hnw-1, cost, 13);
		}
		// w
		if (ok && (h0_nw < hnw || h0_sw < hsw) && ((ctest&9)==9)) {
			const sint8 hw = max(hnw, hsw) -1;
			ok = can_raise_to(x-1,y, keep_water, hw, hsw, hnw, hw, cost, 9);
		}
	}
	else {
...
bool karte_t::can_raise(sint16 x, sint16 y, int &cost) const
{
	...
	if(ist_in_kartengrenzen(x, y)) {
		...
		return can_raise_to(x, y, false, hnew, hnew, hnew, hnew+1, cost, 15/*all corners*/ );
	} else {
		return true;
	}
}
...
{
	int dummy;
	bool ok = can_raise(pos.x, pos.y, dummy);
	int n = 0;
...
bool karte_t::can_lower_to(sint16 x, sint16 y, sint8 hsw, sint8 hse, sint8 hne, sint8 hnw, int &cost, uint8 ctest) const
{
	...
	if(ist_in_kartengrenzen(x,y)) {
		...
		if (ok && h0_sw > hsw) {
			cost += h0_sw-hsw;
			ok = can_lower_to(x-1,y+1, hsw+1, hsw+1, hsw, hsw+1, cost, 11);
		}
		// s
		if (ok && (h0_se > hse || h0_sw > hsw) && ((ctest&3)==3)) {
			const sint8 hs = min(hse, hsw) +1;
			ok = can_lower_to(x,y+1, hs, hs, hse, hsw, cost, 3);
		}
		// se
		if (ok && h0_se > hse) {
			cost += h0_se-hse;
			ok = can_lower_to(x+1,y+1, hse+1, hse+1, hse+1, hse, cost, 7);
		}
		// e
		if (ok && (h0_se > hse || h0_ne > hne) && ((ctest&6)==6)) {
			const sint8 he = max(hse, hne) +1;
			ok = can_lower_to(x+1,y, hse, he, he, hne, cost, 6);
		}
		// ne
		if (ok && h0_ne > hne) {
			cost += h0_ne-hne;
			ok = can_lower_to(x+1,y-1, hne, hne+1, hne+1, hne+1, cost, 14);
		}
		// n
		if (ok && (h0_nw > hnw || h0_ne > hne) && ((ctest&12)==12)) {
			const sint8 hn = min(hnw, hne) +1;
			ok = can_lower_to(x,y-1, hnw, hne, hn, hn, cost, 12);
		}
		// nw
		if (ok && h0_nw > hnw) {
			cost += h0_nw-hnw;
			ok = can_lower_to(x-1,y-1, hnw+1, hnw, hnw+1, hnw+1, cost, 13);
		}
		// w
		if (ok && (h0_nw > hnw || h0_sw > hsw) && ((ctest&9)==9)) {
			const sint8 hw = min(hnw, hsw) +1;
			ok = can_lower_to(x-1,y, hw, hsw, hnw, hw, cost, 9);
		}
	}
	else {
		if (x<0) ok = hne >= grundwasser && hse >= grundwasser;
...
bool karte_t::can_lower(sint16 x, sint16 y, int &cost) const
{
	if(ist_in_kartengrenzen(x, y)) {
		grund_t *gr = lookup_kartenboden(koord(x,y));
		const sint8 hnew = gr->get_hoehe() + corner4(gr->get_grund_hang());

		return can_lower_to(x, y, hnew, hnew, hnew, hnew-1, cost, 15/*all corners*/ );
	} else {
		return true;
	}
}
...
{
	int dummy;
	bool ok = can_lower(pos.x, pos.y, dummy);
	int n = 0;
	if(ok && ist_in_kartengrenzen(pos)) {
		grund_t *gr = lookup_kartenboden(pos);
...
bool karte_t::can_ebne_planquadrat(koord pos, sint8 hgt, int &cost, bool keep_water, bool make_underwater_hill) const
{
	const grund_t *gr = lookup_kartenboden(pos);
	const sint8 old_hgt = make_underwater_hill  &&  gr->ist_wasser() ? min_hgt(pos) : gr->get_hoehe();
	if (old_hgt>=hgt) {
		return can_lower_to(pos.x, pos.y, hgt, hgt, hgt, hgt, cost);
	}
	else {
		return can_raise_to(pos.x, pos.y, keep_water, hgt, hgt, hgt, hgt, cost);
	}
}
...
bool karte_t::ebne_planquadrat(spieler_t *sp, koord pos, sint8 hgt, bool keep_water, bool make_underwater_hill)
{
	...
	const sint8 old_hgt = make_underwater_hill  &&  gr->ist_wasser() ? min_hgt(pos) : gr->get_hoehe();
	if(  old_hgt>=hgt  ) {
		if(  can_lower_to(pos.x, pos.y, hgt, hgt, hgt, hgt, dummy)  ) {
			n = lower_to(pos.x, pos.y, hgt, hgt, hgt, hgt);
			ok = true;
		}
	}
	else {
		if(  can_raise_to(pos.x, pos.y, keep_water, hgt, hgt, hgt, hgt, dummy)  ) {
